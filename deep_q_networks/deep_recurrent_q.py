...
# env = gameEnv(partial=False,size=9) # partial is FOV
...
path = "./center" #T he path to save our model to.
...
# Define the cells for the primary and target q-networks
...
# Set rate of raandom action descrease
e = startE
...
# Make a path for the model to save
if not os.path.exists(path):
    os.makedirs(path)
    os.makedirs(path+"/frames")

# Write first line to of the master log file for control center
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        episodeBuffer = []
        # Reset env and get new observation
        sP = env.reset()
        s = processState(sP)
        d = False
        rAll = 0
        j = 0
        state = (np.zeros([1,h_size]),np.zeros([1,h_size])) # Reset recurrent layer hidden state

        # The Q-Network
        while j < max_epLength:
            j += 1
            # Choose greedy action or random
...
# Make a path for out model to be saved
if not os.path.exists(path):
    os.makedirs(path)
