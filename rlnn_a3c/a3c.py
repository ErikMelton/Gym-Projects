...
# Use to set worker network parameters to those of global network
...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            # Only the worker network need ops for loss functions and graident updating
            if scope != 'global':
...
class Worker():
    ...
    def train(self,rollout,sess,gamma,bootstrap_value):
        ...
        # Here we take the rewards and values from the rollout, and use them to generate the advantage and discounted returns. The advantage function uses "Generalized Advantage Estimation"
        ...
        # Update the global network using gradients from loss. Generate network statistics to periodically save
...
with tf.device("/cpu:0"):
    ...
    num_workers = multiprocessing.cpu_count() # Set workers to number avail CPU threads
    workers = []
...
with tf.Session() as sess:
    ...
    # This is where the async magic happens. Start the work process for each worker in a seperate thread
    worker_threads = []
